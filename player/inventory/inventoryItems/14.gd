...
var name = "Simple Underwear"
var type = "Clothing"
var value = 6
var description = "Underwear made of simple material that adds +1 to role reputation"
...
func when_equiped(res: CharacterMod):
	...
	if is_equiped.has(res):
		return
	res.lower_body_wearable.add(CharacterTag.LOWER_BODY_WEARABLE.IS_UNDERWEAR)
...
func when_unequiped(res: CharacterMod):
	if not is_equiped.has(res):
		return
	res.lower_body_wearable.remove(CharacterTag.LOWER_BODY_WEARABLE.IS_UNDERWEAR)
	res.role_reputation -= 1
	current_quantity += 1
